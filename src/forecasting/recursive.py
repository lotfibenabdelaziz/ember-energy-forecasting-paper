...
from src.forecasting.forecasters import (
    clean_X, extrapolate_exog, forecast_statistical
)
    ...
    for col in exog_future:
        ...
        if k in all_feature_cols:
            if n_ex >= 2 and full_exog[n_ex - 2] != 0:
                row[k] = (full_exog[n_ex-1] - full_exog[n_ex-2]) / abs(full_exog[n_ex-2]) * 100
            else:
                row[k] = 0.0
...
def point_forecast(
    history_df, model_name, all_feature_cols, exog_cols,
...
def get_insample_residuals(
    history_df, model_name, all_feature_cols, exog_cols,
    params, target, ml_cls_map, n_eval=8,
) -> np.ndarray:
    residuals = []
    n     = len(history_df)
