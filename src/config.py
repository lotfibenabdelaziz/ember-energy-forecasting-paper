...
class Config:
    # ── Data ──────────────────────────────────────────────────────────────────
    csv_path: str = field(
        default_factory=lambda: _str("CSV_PATH", "data/raw/yearly_full_release_long_format.csv")
    )
    ...
    mlflow_uri: str = field(
        default_factory=lambda: _str("MLFLOW_TRACKING_URI", "http://localhost:5000")
    )
    mlflow_experiment: str = field(
        default_factory=lambda: _str("MLFLOW_EXPERIMENT", "ember-demand-forecasting")
    )
