...
def fig2_mape_heatmap():
    """Figure 2: MAPE heatmap all models × all countries (single column)."""
    ...
    sns.heatmap(pivot, annot=True, fmt=".1f", cmap="RdYlGn_r",
                ax=ax, linewidths=0.4, cbar_kws={"label": "MAPE (%)"})
...
def fig3_forecast_comparison():
    ...
    for i, country in enumerate(COUNTRIES):
        ...
        ax.plot(hist["Year"], hist["Demand"],
                color="#555", lw=1.2, label="Historical")
        ax.plot(cl["Year"], cl["Forecast"],
                color="#1f77b4", lw=1.5, ls="--", marker="o", ms=3, label="Classical")
        if "Forecast" in dl.columns:
            ax.plot(dl["Year"], dl["Forecast"],
                    color="#2ca02c", lw=1.5, ls="--", marker="s", ms=3, label="DL")
        if "Lower_90" in cl.columns:
            ax.fill_between(cl["Year"], cl["Lower_90"], cl["Upper_90"],
    ...
    axes[-1].set_visible(False)
    fig.suptitle("Electricity Demand Forecast 2025–2030: Classical vs Deep Learning",
...
def fig4_cagr_bar():
    ...
    bars = ax.bar(df["Country"], df["CAGR 24-30 %"],
                  color=COLORS[:len(df)], edgecolor="white", linewidth=0.5)
    for bar, val in zip(bars, df["CAGR 24-30 %"]):
...
def fig5_classical_vs_dl_scatter():
    ...
    cl_best = cl.loc[cl.groupby("Country")["MAPE"].idxmin()][["Country","MAPE"]].rename(columns={"MAPE":"Classic_MAPE"})
    dl_best = dl.loc[dl.groupby("Country")["MAPE"].idxmin()][["Country","MAPE"]].rename(columns={"MAPE":"DL_MAPE"})
    merged  = cl_best.merge(dl_best, on="Country")

    fig, ax = plt.subplots(figsize=(IEEE_SINGLE_COL, 3.0))
    for i, row in merged.iterrows():
        ax.scatter(row["Classic_MAPE"], row["DL_MAPE"],
                   color=COLORS[i % len(COLORS)], s=60, zorder=3)
        ax.annotate(row["Country"], (row["Classic_MAPE"], row["DL_MAPE"]),
