...
def predict_one_step(
    model, X_window: np.ndarray, scaler_y, device: torch.device
) -> float:
    """
...
def walk_forward_dl(
    ...
    for t_year in test_years:
        ...
        loader_tr, loader_val, scaler_X, scaler_y = make_loaders(
            df_tr, feature_cols, target, seq_len,
        ...
        results.append({
            "Year": t_year, "y_actual": y_true, "y_pred": y_pred,
            "tr_loss": tr_losses, "val_loss": val_losses,
        })
            ...
            try:
                ...
                for r in results:
                    all_dl_results.append({
                        ...
                        "y_pred":   r["y_pred"],
                        "abs_pct_error": (
                            abs(r["y_actual"] - r["y_pred"])
                            / (abs(r["y_actual"]) + 1e-9) * 100
                        ),
                    })
                ...
                log.info("  %-10s done (%d steps, avg err %.1f%%)",
                         country, len(results), avg_err)
...
def compute_best_models(dl_metrics: pd.DataFrame) -> pd.DataFrame:
    ...
    return (
        dl_metrics
        .loc[dl_metrics.groupby("Country")["MAPE"].idxmin()]
...
def compare_with_classical(
    ...
    classic_best = (
        classical_benchmark
        .loc[classical_benchmark.groupby("Country")["MAPE"].idxmin(),
             ["Country", "Model", "MAPE"]]
