...
def plot_walk_forward(
    ...
    for i, country in enumerate(countries):
        ax = axes[i]
        sub_actual = (
            dl_res[dl_res["Country"] == country]
            .drop_duplicates("Year")
            .sort_values("Year")
        )
        ax.plot(
            sub_actual["Year"], sub_actual["y_actual"],
            "k-o", lw=2, ms=5, label="Actual", zorder=5,
        )
        ...
        for model_name in model_names:
            ...
            ax.plot(
                ...
                lw=1.5, ls="--", marker="s", ms=4, label=model_name,
            )
...
def plot_mape_heatmap(dl_metrics: pd.DataFrame, fig_dir: str) -> None:
    ...
    """
    pivot = dl_metrics.pivot_table(
        index="Country", columns="Model", values="MAPE"
    )
...
def plot_dl_vs_classical(compare_df: pd.DataFrame, fig_dir: str) -> None:
    ...
    bars1 = ax.bar(x - width / 2, compare_df["Classic_MAPE"], width,
                   label="Best Classical", color="#3b9eff", alpha=0.85)
    bars2 = ax.bar(x + width / 2, compare_df["DL_MAPE"], width,
                   label="Best DL",        color="#00d4a0", alpha=0.85)

    ax.set_xticks(x)
    ax.set_xticklabels(compare_df["Country"], rotation=15, ha="right")
    ax.set_ylabel("MAPE (%)")
    ax.set_title("Best Classical vs Best DL Model — Test MAPE (%)",
...
def plot_dl_forecast(
    ...
    for i, country in enumerate(countries):
        ...
        ax.plot(hist["Year"], hist[TARGET],
                "k-o", lw=2, ms=4, label="Historical")
        ax.plot(fc["Year"], fc["Forecast"],
                color=PALETTE.get(country, "#3b9eff"),
                lw=2.5, ls="--", marker="^", ms=6, label="DL Forecast")

        if "Lower_90" in fc.columns and "Upper_90" in fc.columns:
            ax.fill_between(
                fc["Year"], fc["Lower_90"], fc["Upper_90"],
                alpha=0.15, color=PALETTE.get(country, "#3b9eff"),
                label="90% CI",
            )
    ...
    fig.suptitle("Electricity Demand Forecast 2025–2030 (DL Models)",
                 fontsize=13, fontweight="bold")
    plt.tight_layout()
...
def plot_model_timing(model_timing: dict, fig_dir: str) -> None:
    ...
    bars = ax.bar(names, times,
                  color=[MODEL_COLORS.get(n, "#999") for n in names], alpha=0.85)
