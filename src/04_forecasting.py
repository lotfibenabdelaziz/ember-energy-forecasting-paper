"""
src/04_forecasting.py — Future Demand Forecasting (2025-2030)
================================================================
    ...
    for col in exog_future:
        ...
        if k in all_feature_cols:
            if n_ex >= 2 and full_exog[n_ex - 2] != 0:
                row[k] = (
                    (full_exog[n_ex - 1] - full_exog[n_ex - 2])
                    / abs(full_exog[n_ex - 2]) * 100
                )
            else:
                row[k] = 0.0
...
def forecast_statistical(series: np.ndarray, model_name: str, horizon: int) -> np.ndarray:
    ...
    if model_name in ("Naive", "Naïve", "LinearTrend"):
        ...
        return m.predict(
            np.arange(len(series), len(series) + horizon).reshape(-1, 1)
        ).flatten()
...
def point_forecast(
    ...
    return forecast_ml_recursive(
        history_df, cls, all_feature_cols, exog_cols, horizon, target,
        scaler=scaler, **kw,
    )
...
def compute_growth_summary(
    ...
    for country in COUNTRIES:
        ...
        fc_2025 = fc_df[(fc_df["Country"] == country) & (fc_df["Year"] == 2025)]["Forecast"].values[0]
        fc_2030 = fc_df[(fc_df["Country"] == country) & (fc_df["Year"] == 2030)]["Forecast"].values[0]
        lo_2030 = fc_df[(fc_df["Country"] == country) & (fc_df["Year"] == 2030)]["Lower_90"].values[0]
        hi_2030 = fc_df[(fc_df["Country"] == country) & (fc_df["Year"] == 2030)]["Upper_90"].values[0]
