...
def load_filtered(input_dir: str, source: str = "csv", db_path: str | None = None) -> pd.DataFrame:
    ...
    log.info("Subcategories: %d → %s", df.Subcategory.nunique(), sorted(df.Subcategory.unique()))
    return df
...
def pivot_wide(df: pd.DataFrame) -> tuple[pd.DataFrame, list[str], list[str]]:
    ...
    df_wide.columns = [c.strip().replace(" ", "_").replace("/", "_") for c in df_wide.columns]
...
def plot_check1_missing(df_wide: pd.DataFrame, all_subs: list[str], fig_dir: str) -> None:
    """Mirrors notebook Cell 5."""
    miss_pct = df_wide.groupby("Area")[all_subs].apply(lambda g: g.isnull().mean() * 100)
    ...
    ax.set_title("Check 1 — Missing % per Country × Feature (Raw)", fontsize=12, fontweight="bold")
    plt.tight_layout()
...
# ── Cell 9: Check 2 plot ──────────────────────────────────────────────────────
def plot_check2_timeseries(df_wide: pd.DataFrame, all_subs: list[str], fig_dir: str) -> None:
    ...
    for i, country in enumerate(COUNTRIES):
        ...
        for j, col in enumerate(all_subs):
            ...
            ax.set_title(
                f"{col}\n{country}", fontsize=6.5, fontweight="bold" if col == TARGET else "normal"
            )
...
def plot_check3_coverage(df_wide: pd.DataFrame, fig_dir: str) -> None:
    ...
    pivot_cov = df_wide.pivot_table(index="Area", columns="Year", values=TARGET, aggfunc="count")
    ...
    ax.set_title("Check 3 — Coverage Matrix (Country × Year)", fontsize=12, fontweight="bold")
    plt.tight_layout()
...
def plot_check4_boxplots(
    ...
    for j, col in enumerate(all_subs):
        axes[0][j].boxplot(
            df_wide[col].dropna(), patch_artist=True, boxprops={"facecolor": "#ffcccb"}
        )
        axes[0][j].set_title(
            f"{col}\n(Before)", fontsize=8, fontweight="bold" if col == TARGET else "normal"
        )
        axes[1][j].boxplot(
            df_clean[col].dropna(), patch_artist=True, boxprops={"facecolor": "#cce5ff"}
        )
        axes[1][j].set_title(
            f"{col}\n(After)", fontsize=8, fontweight="bold" if col == TARGET else "normal"
        )
        for r in [0, 1]:
    ...
    fig.suptitle("Check 4 — Outlier Treatment: Before vs After", fontsize=12, fontweight="bold")
    plt.tight_layout()
...
def engineer_features(
    ...
    df_feat[yoy_cols + ratio_cols] = (
        df_feat[yoy_cols + ratio_cols].replace([np.inf, -np.inf], np.nan).fillna(0)
    )
    log.info("NaN in yoy cols after fix: %d", df_feat[yoy_cols].isnull().sum().sum())
...
def plot_check5_heatmap(df_feat: pd.DataFrame, all_subs: list[str], fig_dir: str) -> None:
    ...
    ax.set_title("Check 5 — Subcategory Correlation Matrix", fontsize=12, fontweight="bold")
    plt.tight_layout()
    savefig(fig, os.path.join(fig_dir, "check5_corr_heatmap.pdf"))


def plot_check5_pairplot(df_feat: pd.DataFrame, all_subs: list[str], fig_dir: str) -> None:
...
def save_outputs(
    ...
    df_model.to_csv(os.path.join(output_dir, "ember_model_ready.csv"), index=False)
