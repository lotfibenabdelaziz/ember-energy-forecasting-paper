...
# Subcategory names AS THEY APPEAR AFTER RENAMING (spaces→_, slashes→_)
SUBCATS_RENAMED = [
    "Demand", "CO2_intensity", "Total", "Fuel", "Electricity_imports"
]
# Original names as they appear in ember_filtered.csv
SUBCATS_RAW = [
...
class TestFiCols:
    ...
    def test_fi_cols_all_present_in_model_ready(self, feature_meta):
        """Every feature in all_features must be a column in ember_model_ready.csv"""
        meta, meta_path, out_dir = feature_meta
        csv_path = os.path.join(out_dir, "ember_model_ready.csv")
        if not os.path.exists(csv_path):
            pytest.skip("ember_model_ready.csv not found in fixture")
        df = pd.read_csv(csv_path)
        for feat in meta["all_features"]:
            assert feat in df.columns, f"Feature '{feat}' in all_features but not in CSV"
